...
const script = `// Turn the hash into hash pairs.
...
const main = async () => {
    ...
    const DropGenerativeArt = await ethers.getContractFactory("DropGenerativeArt");
    const dropgen = await DropGenerativeArt.deploy("DropGenArt", "DGA", admin.address, 0, admin.address, script);
    await dropgen.deployed();
    console.log("deployed at: ", dropgen.address);
}
